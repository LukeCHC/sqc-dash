...
class outputEph_():    
    ...
    def select(self, num):
        sys    = self.config['SYS']
        selSV  = self.config['ISV']
        selSVs = list(set([x[0] for x in selSV]))
        sys    = list(set(sys+selSVs))
        sys    = list(filter(None, sys))
        ...
        for i in range(len(BRDCData)):
            if BRDCData[i][0]=='G':
                gBuf.extend(GPSEphParam(BRDCData, i).res())
            if BRDCData[i][0]=='R':
                rBuf.extend(GLOEphParam(BRDCData, i).res())
            if BRDCData[i][0]=='C':
                cBuf.extend(BDSEphParam(BRDCData, i).res())
            if BRDCData[i][0]=='E':
                eBuf.extend(GALEphParam(BRDCData, i).res())
