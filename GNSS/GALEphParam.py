...
class GALEphParam():
    ...
    def __init__(self, blockData, num):
        ...
        # Broadcast orbit - 1
        self.IODE   = float(self.blockData[self.num+1][4:23])
        ...
        self.dataSource  = float(self.blockData[self.num+5][23:42])
        self.Toe_GALWeek = float(self.blockData[self.num+5][42:61])
        # self.spare1      = float(self.blockData[self.num+5][61:80])
        
        # Broadcast orbit - 6
        self.SISA     = float(self.blockData[self.num+6][4:23])
        self.SVhealth = float(self.blockData[self.num+6][23:42])
        self.BGD1     = float(self.blockData[self.num+6][42:61])
        self.BGD2     = float(self.blockData[self.num+6][61:80])

        # Broadcast orbit - 7
        self.TransmissionTime = float(self.blockData[self.num+7][4:23])
        # self.spare2         = float(self.blockData[self.num+7][23:42])
    ...
    def res(self):
        ...
        resList = [
            ...
            self.omega, self.omegaDOT, self.IDOT, self.dataSource, 
            self.Toe_GALWeek, self.SISA, self.SVhealth, self.BGD1, self.BGD2, 
            self.TransmissionTime]
        return resList
